...
namespace pcx
{
    ...
    bool ParseConfig::IsSpace(char c)
    {
        if (' ' == c || '\t' == c)
            return true;
        return false;
    ...
    bool ParseConfig::IsCommentChar(char c)
    {
        switch (c) {
            case '#':
                return true;
    ...
    bool ParseConfig::AnalyseLine(const std::string & line, std::string& section, std::string & key, std::string & value)
    {
        if (line.empty())
            return false;
    ...
    bool ParseConfig::ReadConfig(const std::string & filename)
    {
        settings_.clear();
    ...
    std::string ParseConfig::ReadString(const char* section, const char* item, const char* default_value)
    {
    ...
    int ParseConfig::ReadInt(const char* section, const char* item, const int& default_value)
    {
        std::string tmp_s(section);
        std::string tmp_i(item);
        std::map<std::string, std::string> k_v;
    ...
    float ParseConfig::ReadFloat(const char* section, const char* item, const float& default_value)
    {
        std::string tmp_s(section);
        std::string tmp_i(item);
        std::map<std::string, std::string> k_v;
